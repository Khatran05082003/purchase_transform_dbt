WITH dim_product AS (
  SELECT 
      *
    FROM {{ref("stg_dim_product")}} as stg_production_product
    LEFT JOIN {{ref("stg_dim_productmodel")}} as stg_production_productmodel
    ON stg_production_product.product_model_key_product = stg_production_productmodel.product_model_key
    LEFT JOIN {{ref("stg_dim_productsubcategory")}} as stg_production_productsubcategory
    ON stg_production_product.product_subcategory_key_product = stg_production_productsubcategory.product_subcategory_key
    LEFT JOIN {{ref("stg_dim_productcategory")}} as stg_production_productcategory
